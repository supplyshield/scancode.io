...
from scanpipe.libinv_models import SafeActionable
from scanpipe.libinv_models import ActionablePackageAvailableVersions
from scanpipe.libinv_models import RepositoryActionablePackageAvailableVersionsAssociation
import json
import string
import logging
import requests
...
class FindActionables(Pipeline):
    ...
    def find_paths_to_vulnerable_packages(self):
        ...
        """
        logger.info ("Starting find_paths_to_vulnerable_packages..")
    ...
    def generate_actionables(self):
        ...
        for path in self.all_vulnerable_paths:
            ...
            if self.has_commons_in_path(path, self.parent_namespace):
                ...
                self.unique_actionables.add(actionable)
    ...
    def store_actionables(self):
        ...
        for actionable in self.unique_actionables:
            package_url_object = PackageURL.from_string(actionable)
            package_url_without_version = PackageURL(
                type=package_url_object.type,
                namespace=package_url_object.namespace,
                name=package_url_object.name,
            )
            obj, created = SafeActionable.objects.get_or_create(
                package_url = package_url_without_version.to_string(),
            )
            obj, created = ActionablePackageAvailableVersions.objects.update_or_create(
                package_url = package_url_without_version.to_string(),
                version=package_url_object.version,
                actionable_id = obj.uuid,
                defaults={
                    "is_version_in_use": True
                }                
            )

            obj, created = RepositoryActionablePackageAvailableVersionsAssociation.objects.get_or_create(
                repository_id = self.repository_id,
                actionable_package_available_version = obj,
                environment=self.scan_environment,
